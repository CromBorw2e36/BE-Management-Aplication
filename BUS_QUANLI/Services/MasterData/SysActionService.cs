...
{
    public class SysActionService : rootCommonService, ISysActionService
    {
        ...
        public async Task<StatusMessage<SysAction>> SysActionInsert(SysAction p, HttpRequest httpRequest)
        {
            ...
            var result = systemContext.Database.SqlQueryRaw<StatusMessage<dynamic>>(
        ...
        {
            SysAction get_action = await SysActionGetByCode(p.code);
            if (get_action != null)
            {
                try
                {
